...
class P_Mdetect{
     ...
     * @return bool
     */
    public function parseAgent(&$result=array()){
        $userAgent = $this->getUserAgent();
        $regex = '/^(.+)\/[a-zA-z]?([\d.]+)\s?\((.+)[;]\s?(\w+)\s?([\d.]+)[;]?\s?.*\)\s?$/';
        $rs =  preg_match($regex, $userAgent, $matches);
        if(false == $rs)
            return false;
        $result = array(
            'app_name'      => $matches[1],
            ...
            'dev_name'      => $matches[3],
            'os_name'       => $matches[4],
            'dev_version'   => $matches[5],
        );
        $this->appAgent = $result;
        return true;
    }
