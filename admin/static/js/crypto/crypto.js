...
function make_key(len){
    var key_chars = '12345678abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    var maxPos = key_chars.length;
    var pwd = '';
    for (var i = 0; i < len; i++) {
        pwd += key_chars.charAt(Math.floor(Math.random() * maxPos));
    }
    return pwd;
}
...
function encrypt(data){
    var key = make_key(8);
    var en_str = encryptByDES(data, key);
//    console.info('encrypt:', key, en_str);
    return key.substr(0,6)+en_str+key.substr(4,4);
}

function decrypt(str){
    var len = str.length;
    var key = str.substr(0, 4)+str.substr(len-4,4);
    var str = str.substr(6, len-10);
//    console.info('decrypt:', key, str);
